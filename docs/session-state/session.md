...
Session state in ASP.NET Framework provided a number of a features that ASP.NET Core does not provide. In order to provide the ability to migrate from ASP.NET Framework to Core, the adapters provide mechanisms to enable populating session state. Some of the differences between framework and core are:
