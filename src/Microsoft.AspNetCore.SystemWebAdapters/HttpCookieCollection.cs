...
#pragma warning restore CA5396
    ...
    {
        response.GetAspNetCoreResponse().OnStarting(static state =>
        {
            var response = (HttpResponse)state;
            var headers = response.GetAspNetCoreResponse().Headers;
            ...
            if (!isShareable && !response.GetAspNetCoreResponse().HttpContext.Request.IsHttps && response.TypedHeaders.CacheControl is { Public: true } cacheControl)
            {
