...
#pragma warning restore CA5396
    ...
    {
        response.GetCoreResponse().OnStarting(static state =>
        {
            var response = (HttpResponse)state;
            var headers = response.GetCoreResponse().Headers;
            ...
            if (!isShareable && !response.GetCoreResponse().HttpContext.Request.IsHttps && response.TypedHeaders.CacheControl is { Public: true } cacheControl)
            {
