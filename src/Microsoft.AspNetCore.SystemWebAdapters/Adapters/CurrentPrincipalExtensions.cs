...
internal static partial class CurrentPrincipalExtensions
{
    ...
    [LoggerMessage(2, LogLevel.Warning, "ClaimsPrincipal.ClaimsPrincipalSelector is already set and will not be overridden")]
    private static partial void LogClaimsPrincipalSelectorAlreadySet(ILogger logger);
    ...
    public static void UseCurrentPrincipal(this IApplicationBuilder app)
    {
        ...
        if (ClaimsPrincipal.ClaimsPrincipalSelector is not null)
        {
            LogClaimsPrincipalSelectorAlreadySet(logger);
            return;
        }
