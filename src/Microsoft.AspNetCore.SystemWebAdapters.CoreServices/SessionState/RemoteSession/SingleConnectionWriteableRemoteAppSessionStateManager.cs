    ...
    : RemoteAppSessionStateManager(serializer, options, remoteAppClientOptions, logger)
{
    protected override async Task<ISessionState> GetSessionDataAsync(string? sessionId, bool readOnly, HttpContextCore callingContext, CancellationToken token)
    {
        ...
        var content = new DelayedSerializedSessionHttpContent(Serializer);
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    private class DelayedSerializedSessionHttpContent : HttpContent
    {
        private readonly TaskCompletionSource<ISessionState> _state;

        public DelayedSerializedSessionHttpContent(ISessionSerializer serializer)
        {
    ...
    private sealed class RemoteSessionState(ISessionState other, HttpRequestMessage request, HttpResponseMessage response, DelayedSerializedSessionHttpContent content, Stream stream) : DelegatingSessionState
    {
